...
const Dtype* data1= bottom[0]->mutable_gpu_data();
          ...
          Dtype* combined=Layer<Dtype>::input[16]->mutable_gpu_data();
 
         Dtype* weight_combined=Layer<Dtype>::input[14]->mutable_gpu_data();

    Dtype * combine=Layer<Dtype>::input1[7]->mutable_gpu_data();

caffe_gpu_add1(Layer<Dtype>::input1[7]->count(),data1,combine,lambda1,lambda2,combined);

caffe_gpu_add1(Layer<Dtype>::input1[9]->count(),this->blobs_[0]->mutable_gpu_data(),Layer<Dtype>::input1[9]->mutable_gpu_data(),lambda1,lambda2,weight_combined);

    Dtype* raw_feature=Layer<Dtype>::raw_feature[0]->mutable_gpu_data();
    Dtype* first_layer_col=Layer<Dtype>::first_layer_col[0]->mutable_gpu_data();
    im2col_gpu(raw_feature, 41, conv_in_height, conv_in_width,
        kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, first_layer_col, 1, 1);

          Dtype* top_data1=Layer<Dtype>::first_layer_top_data[0]->mutable_gpu_data(); 
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, 41*kernel_h*kernel_w,
        1,out_h*out_w,
        (Dtype)1., first_layer_col,weight_combined,
        (Dtype)0., top_data1); 


im2col_gpu(combined, conv_in_channels, conv_in_height, conv_in_width,
        kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w, col_buff, 1, 1);

    caffe_copy(Layer<Dtype>::input1[9]->count(),this->blobs_[0]->mutable_gpu_data(),Layer<Dtype>::input1[9]->mutable_gpu_data());
...
const Dtype *top_diff=top[0]->gpu_diff();
  ...
  caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans,out_h*out_w,1, conv_in_channels_*kernel_h_*kernel_w_,
    ...
    Dtype* first_layer_weight_diff=Layer<Dtype>::first_layer_weight_diff[0]->mutable_gpu_data();
    im2col_gpu(raw_feature, 41, conv_in_height_, conv_in_width_,
        kernel_h_, kernel_w_, pad_h_, pad_w_, stride_h_, stride_w_, first_layer_col, 1, 1);

    caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans,out_h*out_w,1, 41*kernel_h_*kernel_w_,
        (Dtype)1., first_layer_col,second_layer_bottom_diff,
        (Dtype)0., first_layer_weight_diff);
    caffe_gpu_add(this->blobs_[0]->count(), this->blobs_[0]->mutable_gpu_diff(), first_layer_weight_diff, this->blobs_[0]->mutable_gpu_diff());
